        ...
        message += " moves " + d;
            if (charStatus[nr][nc] == '.') {
                        ...
                        if(!t->getOccupied()) {
                          ...
                          || ((t->getObject()->getType() == "DH") && !checkForDragon(nr,nc))) {
                            pc->setPos(nr, nc);
                            t->setOccupied(true);
                            //gold, pickup
    ...
    */
    ...
    for (auto & chmbr : chambers){
        for (auto & enemy : chmbr.enemies){
            if (enemy->getChamberNumber() == pc->getChamberNumber()){
                if (abs(enemy->getCol() - pc->getCol()) <= 1 && abs(enemy->getRow() - pc->getRow()) <= 1){
                    ...
                    if (abs(dr->getDH()->getRow() - pc->getRow()) <= 1 && abs(dr->getDH()->getCol() - pc->getCol()) <= 1) enemy->attack(pc);
                }
                else randomMove(enemy);
            }
